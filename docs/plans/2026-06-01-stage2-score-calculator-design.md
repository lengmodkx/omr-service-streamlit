...
class ScoringConfig:
    ...
    ignore_case: bool = False      # 答案大小写不敏感（默认 False，与旧 app.py 行为一致）
```
...
def test_all_wrong(): ...
def test_all_empty(): ...
def test_multi_status_with_matching_set(): ...   # 替代原 test_all_multi_zero_score
                                            # multi 状态 + 集合匹配 → correct=True, earned=1.0
